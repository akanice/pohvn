...
class Multi_menu {
	 ...
	 * @var string
	 */
	private $menu_label               = 'display_name';	
	...
	// vietth - New Render for adminCP
	private function render_item_option($items, &$html = '', &$extra = '') {	    		
		foreach ($items as $item) {
			$html .= '<option ';
			$value  = $item['id'];
			$html .= 'value="'.$value.'">';
			...
			if ( !$has_children ) {
				$html .= $extra.$item[$this->menu_label].'</option>';
				//$extra = '';
			} else {
				$html .= $extra.$item[$this->menu_label].'</option>';
				$extra .= '-';
				$this->render_item_option($item['children'], $html, $extra);
			}
		}
		$extra = '';
	}
	public function render_admin($config = array(), $divided_items_list = array(), $divider = '') {
		$html  = "";
    	if ( count($this->items) )  {
			$items = $this->prepare_items($this->items);		
			$this->set_divided_items($divided_items_list, $divider);
	        $this->render_item_option($items, $html);
    	}
        return $html;
    } 
}