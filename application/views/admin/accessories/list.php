				...
				<div class="card">
					<div class="content">
						<div class="body collapse in" style="margin: 0 0 15px">
							<a href="<?=site_url('admin/accessories/add')?>" class="btn btn-info btn-fill btn-wd">Thêm mới</a>
						</div>
							...
							<div class="widget-body">
								<table class="table table-striped table-bordered" id="sample_1">
									...
									<?php if($list) foreach ($list as $item){ ?>
										<tr class="odd gradeX">
											<td><?=@$item->id?></td>
											<td><image src="<?=@site_url($item->thumb)?>" style="height:60px"></td>
											<td><?=@$item->name?></td>
											<td><?=@$item->category?></td>
											<td style="text-align: center"><a href="<?=@base_url('admin/accessories/edit/'.$item->id)?>"><i class="fa fa-pencil"></i> Sửa</a> | <a href="<?=@base_url('admin/accessories/delete/'.$item->id)?>" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')" ><i class="fa fa-trash"></i> Xóa</a></td>
										</tr>
										<?php } ?>
									</tbody>
								</table>
